		...
		public static boolean setAlarmThresholdValuesForVM(VMlog vmlog){
			...
			try{
				if(isThresholdValid(vmlog.getCpuThreshold()))
					VmStatisticsDao.setVmPropertyThreshold(vm, "CPU", Integer.parseInt(vmlog.getCpuThreshold().toString()));
				
				if(isThresholdValid(vmlog.getMemoryThreshold()))
					VmStatisticsDao.setVmPropertyThreshold(vm, "MEMORY", Integer.parseInt(vmlog.getMemoryThreshold().toString()));
				
				if(isThresholdValid(vmlog.getNetThreshold()))
					VmStatisticsDao.setVmPropertyThreshold(vm, "NET", Integer.parseInt(vmlog.getNetThreshold().toString()));
				
				if(isThresholdValid(vmlog.getDiskReadThreshold()))
					VmStatisticsDao.setVmPropertyThreshold(vm, "DISKREAD", Integer.parseInt(vmlog.getDiskReadThreshold().toString()));
				
				if(isThresholdValid(vmlog.getDiskWriteThreshold()))
					VmStatisticsDao.setVmPropertyThreshold(vm, "DISKWRITE", Integer.parseInt(vmlog.getDiskWriteThreshold().toString()));
			}
			catch(Exception e){
				return false;
			}
